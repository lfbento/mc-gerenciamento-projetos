...
Inovações:
  ...
  2. Função de Aptidão (Fitness):
     Minimiza a variância diária da mão de obra (sigma^2) e penaliza sobrealocações acima da capacidade.
  3. Gera o gráfico comparativo de nivelamento: Antes (Picos e Vales) vs. Depois (Suave e Nivelado).
"""
...
def executar_nivelamento_bioinspirado(
    ...
    # Carga de recursos (FTEs totais) por tarefa
    demandas_tarefa = []
    for t in tarefas:
    ...
    makespan_nominal = max(ef)

    def simular_perfil_recursos(shifts: np.ndarray) -> Tuple[np.ndarray, List[float], List[float]]:
        """Gera o perfil diário de demanda e as datas de início/fim com os shifts."""
        s_dias = [0.0] * n_t
        f_dias = [0.0] * n_t
        ...
        horizonte = int(np.ceil(max(f_dias))) + 2
        perfil = np.zeros(horizonte)

        for i in range(n_t):
            s = int(np.floor(s_dias[i]))
            f = int(np.ceil(f_dias[i]))
            dem = demandas_tarefa[i]
            perfil[s:f] += dem
    ...
    def avaliar_fitness(shifts: np.ndarray) -> float:
        ...
        perfil, s_dias, f_dias = simular_perfil_recursos(shifts)
        
        # 1. Variância da demanda ativa
        dias_ativos = perfil[perfil > 0]
        media = np.mean(dias_ativos) if len(dias_ativos) > 0 else 1.0
        variancia = np.sum((dias_ativos - media) ** 2)

        # 2. Penalidade forte por exceder a capacidade alvo
        excesso = np.maximum(0.0, perfil - capacidade_alvo_fte)
        sobrecarga_pen = np.sum(excesso ** 2) * 80.0

        # 3. Penalidade por estourar o makespan nominal
        atraso_makespan = max(0.0, max(f_dias) - makespan_nominal)
        penalidade_prazo = atraso_makespan * 500.0

        return variancia + sobrecarga_pen + penalidade_prazo

    # 2. Perfil Inicial (Antes do Nivelamento)
    shifts_zero = np.zeros(n_t)
    perfil_antes, s_antes, f_antes = simular_perfil_recursos(shifts_zero)
    var_antes = np.var(perfil_antes[perfil_antes > 0]) if np.any(perfil_antes > 0) else 1.0
            ...
            if RNG.random() < 0.40:
                ...
                if shift_maximos[gene_mut] > 0:
                    filho[gene_mut] = np.clip(
                        filho[gene_mut] + RNG.normal(0, 1.2),
                        0.0,
    ...
    perfil_depois, s_depois, f_depois = simular_perfil_recursos(melhor_ind)
    var_depois = np.var(perfil_depois[perfil_depois > 0]) if np.any(perfil_depois > 0) else 1.0
    ...
    # 5. Geração do Gráfico Comparativo: Antes vs. Depois do Nivelamento Bioinspirado
    ...
    metricas_nivelamento = {
        ...
        "makespan_final_dias": round(max(f_depois), 1)
    }
